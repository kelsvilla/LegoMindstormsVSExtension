			...
			switch (state) {
					...
					if (m1 === "BRICKD") {
						...
						if (compareVersions(version, minBrickdVersion) < 0) {
							...
							this.emit(
								"error",
								new Error(
									`Brickd is too old. Please upgrade to version >= ${minBrickdVersion}`,
								),
							);
							break;
						}
						if (compareVersions(version, maxBrickdVersion) >= 0) {
							...
							this.emit(
								"error",
								new Error("Brickd version is too new."),
							);
							break;
						}
						...
						this.emit(
							"error",
							new Error(
								"Brickd server did not send expected welcome message.",
							),
						);
					}
					break;
				case BrickdConnectionState.handshake:
					if (m1 === "OK") {
						...
						this.emit(
							"error",
							new Error("Brickd handshake failed."),
						);
					}
					break;
				case BrickdConnectionState.watchPower:
					if (m1 === "OK") {
						...
						this.emit(
							"error",
							new Error(
								"Brickd failed to register for power events.",
							),
						);
					}
					break;
				case BrickdConnectionState.getBatteryVoltage:
					if (m1 === "OK") {
						this.emit(
							"message",
							`PROPERTY system.battery.voltage ${m2.join(" ")}`,
						);
						...
						this.emit(
							"error",
							new Error("Brickd failed to get battery voltage"),
						);
					}
					break;
				case BrickdConnectionState.getSerialNum:
					if (m1 === "OK") {
						...
						this.emit(
							"error",
							new Error("Brickd failed to get serial number"),
						);
					}
					break;
			}
