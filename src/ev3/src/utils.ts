...
export async function verifyFileHeader(
	filePath: string,
	expectedHeader: Buffer | number[],
	offset: number = 0,
): Promise<boolean> {
	const bufferExpectedHeader = Array.isArray(expectedHeader)
		? Buffer.from(<number[]>expectedHeader)
		: <Buffer>expectedHeader;
	const header = await openAndRead(
		filePath,
		0,
		bufferExpectedHeader.length,
		offset,
	);
