...
class BonjourClient extends events.EventEmitter implements dnssd.Client {
	...
	private readonly ifaceTimer = setInterval(
	...
	private updateInterfaces() {
		...
		for (let i in ifaces) {
			...
			const localLinkAddr = ifaces[i]!.find((v) =>
				v.address.startsWith("fe80:"),
			);
			if (!localLinkAddr) {
				continue;
			}
			const ifaceIndex = (<os.NetworkInterfaceInfoIPv6>localLinkAddr)
				.scopeid;

			// only supporting IPv6 for now
			const addresses = ifaces[i]!.filter(
				(v) => v.internal === false && v.family === "IPv6",
			).map(
				(v) =>
					`${v.address}%${
						process.platform === "win32"
							? (<os.NetworkInterfaceInfoIPv6>v).scopeid
							: i
					}`,
			);
			newAddresses.push(
				...addresses.map(
					(v) => <Address>{ iface: ifaceIndex, address: v },
				),
			);
		}
		const added = newAddresses.filter(
			(a) => this.ifaceAddresses.indexOf(a.address) === -1,
		);
		const removed = this.ifaceAddresses.filter(
			(a) => newAddresses.findIndex((v) => v.address === a) === -1,
		);
		if (added.length) {
...
class BonjourBrowser extends events.EventEmitter implements dnssd.Browser {
	...
	constructor(
		...
		private readonly opts: dnssd.BrowseOptions,
	) {
		super();
	...
	private addBrowser(bClient: bonjour.Bonjour) {
		...
		browser.on("up", (s) => {
			...
			for (const b of this.browsers) {
				for (const bs of b.services) {
					...
					if (
						(<any>s)["iface"] === (<any>bss)["iface"] &&
						s.name === bs.name &&
						s.type === bss.type &&
						s.fqdn === bss.fqdn.replace(/\.$/, "")
					) {
						// ignore duplicates
						return;
					}
		...
		browser.on("down", (s) => {
			const index = services.findIndex((v) => v.bService.name === s.name);
...
class BonjourService implements dnssd.Service {
	...
	constructor(
		public readonly bService: bonjour.Service | bonjour.RemoteService,
	) {
