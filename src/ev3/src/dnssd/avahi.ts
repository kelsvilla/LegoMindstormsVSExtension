...
interface Server extends dbus.ClientInterface {
	...
	on(
		event: "StateChanged",
		listener: (state: number, error: string) => void,
	): this;
...
async function getServer(): Promise<ServerObject> {
	if (cachedServer === undefined) {
		...
		const iface = proxy.getInterface<Server>(
...
class AvahiBrowser extends events.EventEmitter implements dnssd.Browser {
	...
	constructor(
		private readonly client: AvahiClient,
		...
		(this.bus as any).on("message", (msg: dbus.Message) => {
			...
			switch (msg.member) {
				case "ItemNew":
					{
						const [iface, protocol, name, type, domain, flags] =
							msg.body;
						client.server.iface
							.ResolveService(
								iface,
							...
							.then(
								([
									...
									const service = new AvahiService(
										iface,
										protocol,
										name,
										type,
				...
				case "ItemRemove":
					{
						const [iface, protocol, name, type, domain, flags] =
							msg.body;
						const i = this.services.findIndex((s) =>
							s.match(iface, protocol, name, type, domain),
						);
						if (i >= 0) {
		...
		const addMatchMessage = new dbus.Message({
			...
			signature: "s",
			body: [
				`type='signal',sender='org.freedesktop.Avahi',interface='org.freedesktop.Avahi.ServiceBrowser'`,
			],
		});
		this.bus
			.call(addMatchMessage)
	...
	public async start(): Promise<void> {
		...
		const type = `_${this.options.service}._${
			this.options.transport || "tcp"
		}`;
		const objPath = await this.client.server.iface.ServiceBrowserNew(
			IF_UNSPEC,
			proto,
			type,
			"",
		...
		this.browser = proxy.getInterface<ServiceBrowser>(
