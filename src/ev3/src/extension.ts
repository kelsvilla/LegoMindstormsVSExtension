...
export function activate(context: vscode.ExtensionContext): void {
	...
	context.subscriptions.push(
		output,
		ev3devBrowserProvider,
		vscode.window.registerTreeDataProvider(
			"ev3devBrowser",
			ev3devBrowserProvider,
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.openSshTerminal",
			(d) => d.openSshTerminal(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.captureScreenshot",
			(d) => d.captureScreenshot(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.showSysinfo",
			(d) => d.showSysinfo(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.reconnect",
			(d) => d.connect(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.connectNew",
			(d) => pickDevice(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.disconnect",
			(d) => d.disconnect(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.deviceTreeItem.select",
			(d) => d.handleClick(),
		),
		vscode.commands.registerCommand("ev3devBrowser.fileTreeItem.run", (f) =>
			f.run(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.fileTreeItem.runInTerminal",
			(f) => f.runInTerminal(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.fileTreeItem.delete",
			(f) => f.delete(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.fileTreeItem.showInfo",
			(f) => f.showInfo(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.fileTreeItem.upload",
			(f) => f.upload(),
		),
		vscode.commands.registerCommand(
			"ev3devBrowser.fileTreeItem.select",
			(f) => f.handleClick(),
		),
		vscode.commands.registerCommand("ev3devBrowser.action.pickDevice", () =>
			pickDevice(),
		),
		vscode.commands.registerCommand("ev3devBrowser.action.download", () =>
			downloadAll(),
		),
		vscode.commands.registerCommand("ev3devBrowser.action.refresh", () =>
			refresh(),
		),
		vscode.debug.onDidReceiveDebugSessionCustomEvent((e) =>
			handleCustomDebugEvent(e),
		),
	...
	vscode.window.showInformationMessage("EV3 finished loading!");
}
	...
	implements vscode.DebugConfigurationProvider
{
	async resolveDebugConfiguration(
		...
		if (Object.keys(debugConfiguration).length === 0) {
			type DebugConfigurationQuickPickItem = vscode.QuickPickItem & {
			...
			if (selected) {
				return {
					...
					request: "launch",
					program:
						"/home/robot/${workspaceFolderBasename}/${relativeFile}",
				...
				vscode.window
					...
					.then((value) => {
						if (value === troubleshoot) {
							const wiki = vscode.Uri.parse(
								"https://github.com/ev3dev/vscode-ev3dev-browser/wiki/Troubleshooting",
							);
...
async function handleCustomDebugEvent(
	...
	switch (event.event) {
		case "ev3devBrowser.debugger.launch":
			...
			if (!device || !device.isConnected) {
				await event.session.customRequest(
					"ev3devBrowser.debugger.terminate",
				);
				break;
			}
			...
			if (
				args.download !== false &&
				!(folder ? await download(folder, device) : await downloadAll())
			) {
			...
			try {
				...
				if (args.interactiveTerminal) {
					...
					const config = vscode.workspace.getConfiguration(
						`terminal.integrated.env.${getPlatform()}`,
					);
					const termEnv = config.get<string>("TERM");
					const env = {
						...vscode.workspace
							.getConfiguration("ev3devBrowser")
							.get<object>("env"),
						...vscode.workspace
							.getConfiguration("ev3devBrowser")
							.get<object>("interactiveTerminal.env"),
					};
					const ch = await device.exec(command, env, {
						term:
							termEnv || process.env["TERM"] || "xterm-256color",
					});
					const writeEmitter = new vscode.EventEmitter<string>();
					ch.stdout.on("data", (data: string | Buffer) =>
						writeEmitter.fire(String(data)),
					);
					ch.stderr.on("data", (data: string | Buffer) =>
						writeEmitter.fire(String(data)),
					);
					if (debugTerminal) {
					...
					debugTerminal = vscode.window.createTerminal({
						name: `${path.posix.basename(programPath)} on ${
							device.name
						}`,
						pty: {
							onDidWrite: writeEmitter.event,
							open: (
								dim: vscode.TerminalDimensions | undefined,
							) => {
								if (dim !== undefined) {
		...
		case "ev3devBrowser.debugger.stop":
			...
			if (
				activeDebugSessions.has(event.session.id) &&
				device &&
				device.isConnected
			) {
...
async function downloadAll(): Promise<boolean> {
	...
	if (!vscode.workspace.workspaceFolders) {
		vscode.window.showErrorMessage(
			"Must have a folder open to send files to device.",
		);
		return false;
	}
...
async function download(
	...
	const config = vscode.workspace.getConfiguration(
		"ev3devBrowser.download",
		folder.uri,
	);

	const includeFiles = new vscode.RelativePattern(
		folder,
		config.get<string>("include", ""),
	);
	const excludeFiles = new vscode.RelativePattern(
		folder,
		config.get<string>("exclude", ""),
	);
	const projectDir =
		config.get<string>("directory") || path.basename(folder.uri.fsPath);
	...
	return vscode.window.withProgress(
		...
		async (progress, token) => {
			try {
				...
				if (!files.length) {
					const msg =
						"No files selected for download. Please check the ev3devBrowser.download.include and ev3devBrowser.download.exclude settings.";
					// try to make it easy for the user to fix the problem by offering to
					// open the settings editor
					const openSettings = "Open Settings";
					vscode.window
						.showErrorMessage(msg, openSettings)
						.then((result) => {
				...
				for (const f of files) {
					...
					await device.put(f.fsPath, remotePath, mode, (percentage) =>
						reportProgress(
							`${baseProgressMessage} - ${percentage}%`,
						),
					);
				...
				vscode.window.showInformationMessage(
					`Download to ${deviceName} complete`,
				);
			} catch (err: any) {
				vscode.window.showErrorMessage(
...
class Ev3devBrowserProvider
	extends vscode.Disposable
	implements vscode.TreeDataProvider<DeviceTreeItem | File | CommandTreeItem>
{
	private _onDidChangeTreeData: vscode.EventEmitter<
		DeviceTreeItem | File | CommandTreeItem | undefined
	...
	private readonly noDeviceTreeItem = new CommandTreeItem(
		...
		"ev3devBrowser.action.pickDevice",
	);
	...
	public getTreeItem(
		element: DeviceTreeItem | File | CommandTreeItem,
	): vscode.TreeItem {
		return element;
	}
	...
	public getChildren(
		element?: DeviceTreeItem | File | CommandTreeItem,
	): vscode.ProviderResult<(DeviceTreeItem | File | CommandTreeItem)[]> {
		if (!element) {
...
class DeviceTreeItem extends vscode.TreeItem {
	...
	constructor(public readonly device: Device) {
		super(device.name);
		this.command = {
			command: "ev3devBrowser.deviceTreeItem.select",
			title: "",
			arguments: [this],
		};
		device.onWillConnect(() =>
	...
	private handleConnectionState(state: DeviceState): void {
		...
		switch (state) {
			...
			case DeviceState.Connected:
				...
				this.rootDirectory.collapsibleState =
					vscode.TreeItemCollapsibleState.Expanded;
	...
	public openSshTerminal(): void {
		...
		const termEnv = config.get<string>("TERM");
		this.device
			.shell({ term: termEnv || process.env["TERM"] || "xterm-256color" })
			.then((ch) => {
				const writeEmitter = new vscode.EventEmitter<string>();
				ch.stdout.on("data", (data: string | Buffer) =>
					writeEmitter.fire(String(data)),
				...
				ch.on("error", (err: any) => {
					vscode.window.showErrorMessage(
						`SSH connection error: ${err || err.message}`,
					);
					term.dispose();
					ch.destroy();
				});
			...
			.catch((err) => {
				vscode.window.showErrorMessage(
					`Failed to create SSH terminal: ${err || err.message}`,
				);
			});
	}
			...
			(progress): Promise<void> => {
				return new Promise(async (resolve, reject) => {
					const handleCaptureError = (e: any) => {
						vscode.window.showErrorMessage(
							"Error capturing screenshot: " + (e.message || e),
						);
						reject();
					};
					...
					try {
						const screenshotDirectory =
							await getSharedTempDir("ev3dev-screenshots");
						...
						const writeStream =
							fs.createWriteStream(screenshotFile);
						...
						writeStream.on("error", (e: Error) => {
							vscode.window.showErrorMessage(
								"Error saving screenshot: " + e.message,
							);
							reject();
						});
						...
						writeStream.on("finish", async () => {
							...
							if (
								...
								vscode.commands.executeCommand(
									"vscode.open",
									vscode.Uri.file(screenshotFile),
									vscode.ViewColumn.Two,
								);
							} else {
								handleCaptureError(
									"The screenshot was not in the correct format. You may need to upgrade to fbcat 0.5.0.",
								);
							}
						});
					} catch (e) {
	...
	public async showSysinfo() {
		try {
			...
			await vscode.window.withProgress(
				...
				async (progress) => {
					const [stdout, stderr] =
						await this.device.createExecObservable(
							"ev3dev-sysinfo",
						);
					await Promise.all([
						stdout.forEach((v: string) => output.appendLine(v)),
						stderr.forEach((v: string) => output.appendLine(v)),
					]);
				},
			);
			...
			vscode.window.showErrorMessage(
				"An error occurred while getting system info: " +
					(err?.message || err),
			);
		}
	}
	...
	public async connect(): Promise<void> {
		try {
			...
			vscode.window
				.showErrorMessage(
					`Failed to connect to ${this.label}: ${err?.message}`,
					troubleshoot,
				)
				.then((value) => {
					if (value === troubleshoot) {
...
class File extends vscode.TreeItem {
	...
	constructor(
		...
		this.command = {
			command: "ev3devBrowser.fileTreeItem.select",
			title: "",
			arguments: [this],
		};
	}
					...
					if (list) {
						list.forEach((element) => {
							...
							if (element.filename[0] !== ".") {
								const file = this.createOrUpdate(
									this.device,
									this.path + "/",
									element,
								);
	...
	public handleClick(): void {
		...
		if (this.isExecutable) {
			...
			vscode.window
				.showQuickPick([runItem, runInTerminalItem])
				.then((value) => {
					switch (value) {
						case runItem:
							this.run();
	...
	public delete(): void {
		vscode.window.withProgress(
			...
			async (progress) => {
				try {
					const config =
						vscode.workspace.getConfiguration("ev3devBrowser");
					...
					vscode.window.showErrorMessage(
						`Error deleting '${this.path}': ${err.message}`,
					);
				}
			},
		);
	...
	public async showInfo(): Promise<void> {
		...
		output.appendLine("");
		try {
			let [stdout, stderr] = await this.device.createExecObservable(
				`/bin/ls -lh "${this.path}"`,
			);
			await Promise.all([
				stdout.forEach((line: string) => output.appendLine(line)),
				stderr.forEach((line: string) => output.appendLine(line)),
			]);
			output.appendLine("");
			[stdout, stderr] = await this.device.createExecObservable(
				`/usr/bin/file "${this.path}"`,
			);
			await Promise.all([
				stdout.forEach((line: string) => output.appendLine(line)),
				stderr.forEach((line: string) => output.appendLine(line)),
...
class DeviceStatusTreeItem extends CommandTreeItem {
	...
	private batteryItem = new CommandTreeItem(
		this.defaultBatteryLabel,
		undefined,
	);
	...
	public async connectBrickd() {
		...
		try {
					...
					case "PROPERTY":
						switch (m2[0]) {
							case "system.battery.voltage":
								...
								this.batteryItem.label = `Battery: ${voltage.toFixed(
									2,
								)}V`;
			...
			vscode.window.showWarningMessage(
				"Failed to get brickd connection. No status will be available.",
			);
			return;
		}
	}
